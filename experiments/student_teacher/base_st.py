...
@partial(jit, static_argnames=("switch_point", 
                               "t1_w1", "t1_w2", "t2_w1", "t2_w2",
                               "d_in", "sample_rate",
                               "model_apply_fn", "optimizer", 
                               "test_inputs",
                               "t1_test_targets", "t2_test_targets", 
                               "num_epochs"))
def vectorized_train_for_v(
    ...
    test_inputs, # static
    t1_test_targets, # static 
    t2_test_targets,  # static
    num_epochs, # static
    batch_size, # static
    ):
...
if __name__ == "__main__":
    ...
    switch_point = 1_000_000
    lr = 1 # lr is 1 for d_in=10_000, 0.1 for d_in=1_000
    sample_rate = 10_000 # Sample every N steps
    v_values = np.linspace(0, 1, 11) # Reduced for testing
    num_runs = 5
    output_dir = f"./loss_data/tests/plots_flat_head_runs_{num_runs}/"

...
from single_expert import train_experts, scaled_normal_init, teacher_forward
...
def plot_losses(npz_path: str | Path, log_y: bool = True) -> None:
    ...
    f_overlap, b_overlap = overlaps
    b_overlap_mean = np.mean(b_overlap) if len(b_overlap) > 0 else float('nan')
    f_overlap_mean = np.mean(f_overlap) if len(f_overlap) > 0 else float('nan')
    b_overlap_std = np.std(b_overlap) if len(b_overlap) > 0 else float('nan')
    f_overlap_std = np.std(f_overlap) if len(f_overlap) > 0 else float('nan')

    densities = data.get('f_mask_densities', float('nan'))
    dens_mean = np.mean(densities) if len(densities) > 0 else float('nan')
    dens_std = np.std(densities) if len(densities) > 0 else float('nan')
    ...
    test_sems = test_losses.std(axis=1)/jnp.sqrt(test_losses.shape[1])
    ...
    fig.update_layout(
        title_text=(
            ...
            f"Overlap_backwards: mean={b_overlap_mean:.2f}, std={b_overlap_std:.2f},"
            f"Overlap_forwards: mean={f_overlap_mean:.2f}, std={f_overlap_std:.2f},\n"
            f"Forward Mask Density: mean={dens_mean:.2f}, std={dens_std:.2f}"
        ),
...
# 1. FIXED custom_vjp implementation
@jax.custom_vjp
def mask_backward(x, mask_f, mask_b):
    # Standard forward pass - apply forward mask
    return x * mask_f

def mask_backward_fwd(x, mask_f, mask_b):
    # Forward pass returns (output, residuals_for_backward)
    output = x * mask_f
    residuals = (mask_b,)  # Store what we need for backward pass
    return output, residuals

def mask_backward_bwd(residuals, g):
    # Backward pass receives residuals from forward and gradient
    mask_b, = residuals  # Unpack residuals
    # Return gradients for each argument in same order as function signature
    grad_x = g * mask_b  # Apply backward mask to gradient
    grad_mask_f = None   # No gradient for mask_f
    grad_mask_b = None   # No gradient for mask_b
    return (grad_x, grad_mask_f, grad_mask_b)
...
class StudentHead(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, n_units):
        return nn.Dense(
            features=1,
            use_bias=False,
            name="head_out",
            kernel_init=scaled_normal_init(jnp.sqrt(1.0 / x.shape[-1])),
        )(x)
...
class StudentNetwork(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, masks_f: jnp.ndarray, masks_b: jnp.ndarray):
        h = nn.relu(self.layer1(x))

        def apply_mask_and_head(mask_f, mask_b):
            n_units = mask_f.sum()
            norm = jnp.sqrt(jnp.maximum(n_units, 1e-8))
            hidden = mask_backward(h, mask_f, mask_b) / norm
            return self.head_layer(hidden, n_units).squeeze(-1)

        # vmap over both masks
        outputs = vmap(apply_mask_and_head)(masks_f, masks_b)
        return outputs
...
def compute_grads(params, batch, teacher_params, task_idx, apply_fn, masks_f, masks_b):
    """Computes loss and gradients for a single task."""
    def loss_fn(p):
        # Student produces outputs for all tasks
        all_preds = apply_fn({'params': p}, batch, masks_f, masks_b)
...
@partial(jit, static_argnames=("apply_fn",))
def evaluate_metrics(params, apply_fn, test_inputs, all_teacher_targets, masks_f, masks_b):
    """Computes test losses against all teachers."""
    all_preds = apply_fn({'params': params}, test_inputs, masks_f, masks_b)
...
def train_single_task(
    initial_carry: tuple,  # Now only contains (params, opt_state, keys)
    task_idx: int,
    ...
    vmap_compute_grads = vmap(compute_grads, in_axes=(0, 0, 0, None, None, 0, 0))
    vmap_evaluate_metrics = vmap(evaluate_metrics, in_axes=(0, None, None, 0, 0, 0))
    ...
    def step_fn(carry, step_idx):
        params_batch, opt_state_batch, keys_batch = carry  # Removed masks_batch
        ...
        # Compute gradients using the masks passed as arguments
        train_loss_batch, grads_batch = vmap_compute_grads(
            params_batch, batch_data, current_teacher_params, task_idx, 
            model_apply_fn, masks_f, masks_b
        )
        ...
        def perform_eval():
            return vmap_evaluate_metrics(
                new_params_batch, model_apply_fn, test_inputs, 
                all_teacher_test_targets, masks_f, masks_b
            )
        ...
        new_carry = (new_params_batch, new_opt_state_batch, keys_batch)  # Removed masks
...
# ---------------- START: ADDED MASK GENERATION FUNCTION ----------------
# @partial(jit, static_argnames=('hidden_dim',))
def _generate_forward_from_backward_single(mask_b, key, overlap_f, density_f, hidden_dim):
    """
    Generates a single forward mask based on a single backward mask,
    controlled by overlap and density parameters.
    """
    mask_f_key, _ = random.split(key)
    n_active_b = jnp.sum(mask_b)
    n_inactive_b = len(mask_b) - n_active_b
    n_active_f = n_active_b * density_f
    n_active_f_cap_b = jnp.min(jnp.array([n_active_f, n_active_b])) * overlap_f
    n_active_f_not_b = n_active_f * (1 - overlap_f) + jnp.max(jnp.array([n_active_f - n_active_b, 0]))

    P1 = mask_b * (n_active_f_cap_b / n_active_b) # num of active_a
    P0 = (1-mask_b) * (n_active_f_not_b/ n_inactive_b)
    P = P0 + P1
    mask_f = jax.random.bernoulli(mask_f_key, P).astype(jnp.float32)
    
    realized_overlap = jnp.matmul(mask_b, mask_f)/(jnp.linalg.norm(mask_b) * jnp.linalg.norm(mask_f))
    realized_density = jnp.sum(mask_f)/jnp.sum(mask_b)

    return mask_f, realized_overlap, realized_density

def generate_forward_masks_from_backward(masks_b_batch, keys_batch, overlap_f, density_f, hidden_dim):
    """
    Generates a batch of forward masks based on a batch of backward masks.
    
    Args:
        masks_b_batch: Backward masks with shape (num_runs, ntasks, hidden_dim).
        keys_batch: JAX PRNG keys with shape (num_runs, ntasks).
        overlap_f: Target overlap fraction between forward and backward masks' supports.
        density_f: Density of forward mask relative to backward mask's support size.
        hidden_dim: The size of the mask dimension.
        
    Returns:
        A tuple of (forward_masks_batch, mean_realized_overlap).
    """
    # Vmap the single-mask generation function over tasks and then runs
    vmap_over_tasks = vmap(
        _generate_forward_from_backward_single, 
        in_axes=(0, 0, None, None, None)
    )
    vmap_over_runs = vmap(vmap_over_tasks, in_axes=(0, 0, None, None, None))
    masks_f_batch, realized_overlaps, realized_densities = vmap_over_runs(
        masks_b_batch, keys_batch, overlap_f, density_f, hidden_dim
    )
    
    return masks_f_batch, realized_overlaps, realized_densities
# ----------------- END: ADDED MASK GENERATION FUNCTION -----------------
...
def run_training_loop(
    ...
    # --- CHANGE: Renamed keys for clarity ---
    mask_b_keys, mask_f_gen_keys = random.split(mask_keys, 2)

    # vmap mask creation over the number of runs
    vmap_create_masks = vmap(create_masks, in_axes=(None, None, None, None, None, 0, None))

    # --- CHANGE: Generate backward masks first, as specified ---
    masks_b_batch, m_b_overlap = vmap_create_masks(
        d_hs, config['sparsity'], config['similarity'], config['overlap_b'],
        config['g_type'], random.split(mask_b_keys, num_runs), ntasks
    )
    
    # --- CHANGE: Generate forward masks based on the backward masks ---
    # We need a unique key for each mask to be generated.
    f_gen_subkeys = vmap(lambda k: random.split(k, ntasks))(random.split(mask_f_gen_keys, num_runs))
    # TODO: Mask overlaps throwing error when saving
    masks_f_batch, m_f_overlap, m_f_densities = generate_forward_masks_from_backward(
        masks_b_batch,
        f_gen_subkeys,
        config['overlap_f'],
        config['density_f'],
    ...
    m_f_overlap = jnp.mean(m_f_overlap, axis=1)
    m_f_densities = jnp.mean(m_f_densities, axis=1)
    # --- END CHANGE ---
    
    # vmap state creation over the number of runs
    student_net = StudentNetwork(hidden_dim=d_h, d_in=config['d_in'])
    vmap_create_state = vmap(
        lambda k, m_f, m_b: student_net.init(k, jnp.ones((1, d_in)), m_f, m_b)['params'],
        in_axes=(0, 0, 0)
    )
    initial_params_batch = vmap_create_state(
        random.split(model_keys, num_runs), masks_f_batch, masks_b_batch
    )
    ...
    for i in range(ntasks):
        ...
        with Timer(f"  Task {i+1} duration"):
            ...
            current_carry, (train_losses, test_losses) = train_single_task(
                ...
                masks_f=masks_f_batch,  # Add this
                masks_b=masks_b_batch,  # Add this
    ...
    # --- CHANGE: Return the new overlap metric for f-b masks, along with the b-b mask overlap ---
    return sampled_train_loss, sampled_test_losses, (masks_f_batch[0], masks_b_batch[0]), (m_f_overlap, m_b_overlap), m_f_densities


@partial(jit, static_argnames=('K',))
def get_C(v: float, K: int) -> jnp.array:
...
def generate_single_teacher_w1(v: float, U: jnp.ndarray, d_ht: int, d_in: int) -> jnp.ndarray:
    K = U.shape[1]
    C = get_C(v, K)
...
def get_all_teacher_weights(key: jnp.ndarray, num_runs: int, ntasks: int, d_in: int, d_ht: int, similarity: float):
    """
...
def setup_and_run_experiment(config):
    ...
    student_model = StudentNetwork(hidden_dim=config['d_h'], d_in=config['d_in'])
    ...
    all_teacher_params = get_all_teacher_weights(
        ...
        config['d_in'], config['d_ht'], config['similarity']  # Pass float directly
    )
    ...
    with Timer("Total student training and evaluation"):
        train_loss, test_losses, masks, overlaps, f_mask_densities = run_training_loop(
    ...
    masks_f_to_save, masks_b_to_save = masks
    results = {
        ...
        "masks_f": np.array(masks_f_to_save),
        "masks_b": np.array(masks_b_to_save),
        "overlaps": overlaps,
        "f_mask_densities": f_mask_densities,
        **config_to_save
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--overlap_f", type=float, default=1.0, help="Overlap between forward and backward masks")
    parser.add_argument("--overlap_b", type=float, default=0.5, help="Shared units between backward task masks")
    # --- CHANGE: Added density_f argument ---
    parser.add_argument("--density_f", type=float, default=1.0, help="Density of forward mask relative to backward mask support size.")
    ...
    # --- CHANGE: Updated output directory string to include new parameter ---
    output_dir_str = (
        f"{args.path}/ntasks_{args.ntasks}_sparsity_{sparsity:.2f}"
        f"_overlap_f_{args.overlap_f:.2f}_overlap_b_{args.overlap_b:.2f}_density_f_{args.density_f:.2f}"
        f"_gtype_{args.g_type}_v_{args.v}/"
    )
    ...
    config = {
        ...
        "similarity": args.v,
        "overlap_f": args.overlap_f,
        "overlap_b": args.overlap_b,
        "density_f": args.density_f, # Added density_f to config
        "g_type": args.g_type,
        "seed": 42,
